...
评估体系
    ...
    └── Qwen 3.7 Plus Judge → 维度评分（5-6 维度，22 模块）
```
...
- **生成-评估模型解耦**：生成用 DeepSeek V4 Flash，评估用 Qwen 3.7 Plus
...
| 评估类型 | 模块数 | 平均评分 | 评分模型 | 评分维度 |
|---------|--------|---------|---------|---------|
| **手工用例** | 10 模块 | **82.0%** | Qwen 3.7 Plus | 格式规范/规则遵循/场景覆盖/可执行性/数据合理 |
| **AI 系统测试** | 8 模块 | **80.6%** | Qwen 3.7 Plus | 功能/准确性/鲁棒性/用户体验/安全/分析报告 |
| **接口用例(JSON)** | 22 模块 | **DeepEval 评估中** | — | 格式正确性/断言规范性/边界值覆盖/场景覆盖/参数有效性 |
...
| **评估模型** | Qwen 3.7 Plus | 通过阿里百炼 DashScope 调用 |
...
2. **LLM Judge（Qwen 3.7 Plus）**：对手工用例和 AI 用例做维度评分，支持 5-6 个评估维度
