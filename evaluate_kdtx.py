...
# DeepSeek 配置
# ======================
DEEPSEEK_MODEL = os.getenv("LLM_MODEL", "deepseek-v3")
DEEPSEEK_API_KEY = os.getenv("DASHSCOPE_API_KEY")
DEEPSEEK_BASE_URL = os.getenv("LLM_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
...
def evaluate_by_deepseek(cases: List[Dict], module_name: str, required_num: int) -> Dict:
    """使用 DeepSeek 评估用例质量"""
...
{{
    ...
    try:
        ...
        logger.error(f"DeepSeek 评估失败: {e}")
        return {
            "total_score": 0,
            "max_score": 80,
            "percent_score": 0,
            ...
            "suggestions": "请检查 DeepSeek API 配置"
        }
...
def evaluate_module(project_name: str, module_name: str, description: str) -> Dict:
    ...
    # 生成用例
    result = generate_api_test_full(project_name, module_name, "功能测试", required_num, "")
    cases = result.get("cases", [])

    elapsed = time.time() - start_time

    # DeepSeek 评估
    evaluation = evaluate_by_deepseek(cases, module_name, required_num)
